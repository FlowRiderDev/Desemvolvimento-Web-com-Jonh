...
async function renderGallery() {
    console.log('Starting to render gallery...');
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        if (pokemon) {
            ...
            const color = typeColors[type] || '#FFFFFF'; // Default to white if type not in mapping
            ...
            card.addEventListener('mouseout', resetCardHighlight);
    ...
    console.log('Finished rendering gallery.');
}
...
function updateGallery(pokemonList) {
    ...
    pokemonList.forEach((pokemon) => {
        ...
        // **2.1** Get the color for the Pokémon's primary type
        const type = pokemon.types[0].type.name;
        const color = typeColors[type] || '#FFFFFF'; // Default to white if type not in mapping
...
// **17** Map Pokémon types to pastel colors
const typeColors = {
    fire: '#FFD1BA', // Pastel orange
    water: '#BAE1FF', // Pastel blue
    grass: '#D4F0C0', // Pastel green
    electric: '#FFF8BA', // Pastel yellow
    psychic: '#FFB3D9', // Pastel pink
    ice: '#BAFFFF', // Pastel cyan
    dragon: '#C4BAFF', // Pastel purple
    dark: '#D3D3D3', // Pastel gray
    fairy: '#FFCCF9', // Light pink
    normal: '#F5F5DC', // Pastel beige
    fighting: '#FFC4BA', // Light salmon
    flying: '#E3D5FF', // Lavender
    poison: '#D8BFD8', // Thistle
    ground: '#EED9C4', // Sandy beige
    rock: '#C4BBAE', // Earthy tone
    bug: '#E6FFE6', // Light mint
    ghost: '#D7C1E9', // Soft lilac
    steel: '#CCCCCC', // Light gray
};

function highlightCard(hoveredCard) {
    const cards = document.querySelectorAll('.card');
    cards.forEach((card) => {
        if (card !== hoveredCard) {
            card.style.opacity = '0.5'; // Dim other cards
        } else {
            card.style.transform = 'scale(1.1)'; // Slightly enlarge hovered card
        }
    });
}
